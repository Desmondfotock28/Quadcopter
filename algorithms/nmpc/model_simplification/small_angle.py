...
def Quadcopter_ode(x, u):
    ...
    - u: Input vector [u1, u2, u3, u4] (control inputs/thrust on four motors)
    ...
    dwx, dwy, dwz = 0.0, 0.0, 0.0
     ...
     # Equations of motion  with the pitch and roll assuming to be slight 
    dx0 = x[6]
    ...
    dx6 =  (1/(m*sqrt(2)))*(sin(x[4]))*(u[0] + u[1] + u[2] + u[3]) + dwx   #(-kd/m)*x[6] ignore for now
    dx7 = (1/m*sqrt(2))*(sin(x[5])*sin(x[4]))*(u[0] + u[1] + u[2]+u[3]) + dwy   # (-kd/m)*x[7] +

    dx8 = -g + (1/(m*sqrt(2)))*(cos(x[4])*cos(x[3]))*(u[0] + u[1] + u[2]+ u[3]) + dwz  #(-kd/m)*x[8] 

    dx9 = (l/(sqrt(2)*Ixx))*(u[0]- u[1]- u[2] + u[3])-((Iyy-Izz)/Ixx)*x[10]*x[11]

    dx10 = (l/(sqrt(2)*Iyy))*(u[0] +u[1] -u[2] - u[3])-((Izz-Ixx)/Iyy)*x[9]*x[11]

    dx11 = (b/(sqrt(2)*Izz*k))*(u[0] -u[1] +u[2] -u[3])-((Ixx-Iyy)/Izz)*x[9]*x[10]
   
    dx = vertcat(dx0, dx1, dx2, dx3, dx4, dx5, dx6, dx7, dx8, dx9, dx10, dx11)

    return dx

def reference_trajectory(t, omega=np.pi, a=1):
    """
...
# Input constraints
lb_u = np.array([0.025, 0.025, 0.025, 0.025])
ub_u = np.array([11.86, 11.86 , 11.86, 11.86])
umax = 11.86
Opt_Vars = vertcat(
    reshape(X, -1, 1),
    reshape(U, -1, 1)
...
u0 =  np.array([5.92, 5.92 , 5.92, 5.92])
