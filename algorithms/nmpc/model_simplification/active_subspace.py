...
def Quadcopter_ode(x, u):
    ...
    dx6 =  (1/(m*sqrt(2)))*(sin(x[5])*sin(x[3])+cos(x[5])*cos(x[3])*sin(x[4]))*(u[0] + u[1] + u[2] + u[3]) + dwx   #(-kd/m)*x[6] ignore for now
    dx7 = (1/m*sqrt(2))*(cos(x[3])*sin(x[5])*sin(x[4])- cos(x[5])*sin(x[3]))*(u[0] + u[1] + u[2]+u[3]) + dwy   # (-kd/m)*x[7] +
...
N_m = 120  # Number of rows
nv = 60  # Number of columns
