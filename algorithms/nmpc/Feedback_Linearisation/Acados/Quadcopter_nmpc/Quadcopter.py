...
def export_Quadcopter_ode_model() -> AcadosModel:
    ...
    f_expl = vertcat(x[6],
                     ...
                     x[9] + x[10]*(sin(x[3])*tan(x[4])) + x[11]*(cos(x[3])*tan(x[4])),

                     x[10]*(cos(x[3])) - x[11]*(sin(x[3])),

                     (sin(x[3])/cos(x[4]))*x[10] + (cos(x[3])/cos(x[4]))*x[11],

                     (k*cm/m)*(sin(x[5])*sin(x[3])+cos(x[5])*cos(x[3])*sin(x[4]))*(u[0]**2 + u[1]**2 + u[2]**2+u[3]**2) + dwx,   #(-kd/m)*x[6] ignore for now

                     (k*cm/m)*(cos(x[3])*sin(x[5])*sin(x[4])- cos(x[5])*sin(x[3]))*(u[0]**2 + u[1]**2 + u[2]**2+u[3]**2) + dwy,   # (-kd/m)*x[7] 

                    -g + (k*cm/m)*(cos(x[4])*cos(x[3]))*(u[0]**2 + u[1]**2 + u[2]**2+u[3]**2) + dwz,   #(-kd/m)*x[8] 

                     (l*k*cm/Ixx)*(u[0]**2-u[1]**2-u[2]**2 + u[3]**2)-((Iyy-Izz)/Ixx)*x[10]*x[11],

                     (l*k*cm/Iyy)*(u[0]**2+u[1]**2-u[2]**2 - u[3]**2)-((Izz-Ixx)/Iyy)*x[9]*x[11],

                     (b*cm/Izz)*(u[0]**2-u[1]**2+u[2]**2-u[3]**2)-((Ixx-Iyy)/Izz)*x[9]*x[10]
