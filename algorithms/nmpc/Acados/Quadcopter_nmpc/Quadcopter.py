...
def export_Quadcopter_ode_model() -> AcadosModel:
    ...
    dx6 =  (k*cm/m)*(sin(x[5])*sin(x[3])+cos(x[5])*cos(x[3])*sin(x[4]))*(u[0]**2 + u[1]**2 + u[2]**2+u[3]**2)    #(-kd/m)*x[6] ignore for now
    
    dx7 = (k*cm/m)*(cos(x[3])*sin(x[5])*sin(x[4])- cos(x[5])*sin(x[3]))*(u[0]**2 + u[1]**2 + u[2]**2+u[3]**2)    # (-kd/m)*x[7] +

    dx8 = -g + (k*cm/m)*(cos(x[4])*cos(x[3]))*(u[0]**2 + u[1]**2 + u[2]**2+u[3]**2)  #(-kd/m)*x[8] 
