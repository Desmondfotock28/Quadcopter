...
public:
  ...
  : ModeBase(node, kName), _node(node)
  {
    ...
    RCLCPP_INFO(_node.get_logger(), "NMPCFlightMode constructor entered");
    ...
    // Subscribe to VehicleOdometry for angular velocity
    _vehicle_odometry_sub = _node.create_subscription<px4_msgs::msg::VehicleOdometry>(
    "/fmu/out/vehicle_odometry",
    rclcpp::SensorDataQoS(), // <- matches PX4 QoS
  ...
  void updateSetpoint(float dt_s) override
  {
    ...
    RCLCPP_INFO(_node.get_logger(), "updateSetpoint() called!");
  ...
  // Use VehicleOdometry subscription for angular velocity
      Eigen::Vector3f ang_vel_frd = _vehicle_ang_vel_frd;

      RCLCPP_INFO(_node.get_logger(), "Got angular velocity: [%f, %f, %f]",
                  ang_vel_frd(0), ang_vel_frd(1), ang_vel_frd(2));
    ...
    RCLCPP_INFO(_node.get_logger(), "converttion from NED TO ENU DONE!");
...
private:
  ...
  Eigen::Vector3f _vehicle_ang_vel_frd{0.0, 0.0, 0.0};
...
private:
  ...
  void _initOCP(Eigen::Vector3f pos_enu, Eigen::Quaternionf quat_enu, Eigen::Vector3f lin_vel_enu, Eigen::Vector3f ang_vel_flu)
  {
    ...
    ocp_nlp_constraints_model_set(nlp_config, nlp_dims, nlp_in, nlp_out, 0, "lbx", x_init);
    ocp_nlp_constraints_model_set(nlp_config, nlp_dims,  nlp_in, nlp_out, 0, "ubx", x_init);
    ocp_nlp_constraints_model_set(nlp_config, nlp_dims, nlp_in, nlp_out, 0, "lbu", prev_u);
    ocp_nlp_constraints_model_set(nlp_config, nlp_dims, nlp_in, nlp_out, 0, "ubu", prev_u);
  }
